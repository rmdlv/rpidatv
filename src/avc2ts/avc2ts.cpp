...
#define ERR_OMX(err, msg)       \
	...
	public:
		...
		void setFrameStabilisation(OMX_U32 nPortIndex = OMX_ALL, OMX_BOOL bStab = OMX_FALSE)
		{
		...
		void setWhiteBalanceControl(OMX_U32 nPortIndex = OMX_ALL, OMX_WHITEBALCONTROLTYPE eWhiteBalControl = OMX_WhiteBalControlGreyWorld)
		{
...
public:
	...
	void AddFrame(uint8_t *buffer, int size, int OmxFlags, uint64_t key_frame, int DelayPTS = 200, struct timespec *Time = NULL)
	{
		...
		else if (OmxFlags & OMX_BUFFERFLAG_ENDOFFRAME)
